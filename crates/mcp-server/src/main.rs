...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tracing_subscriber::registry()
        .with(fmt::layer())
        .with(EnvFilter::from_env("LOG_LEVEL").unwrap_or_else(|_| {
            format!("opentdf_mcp_server={},tower_http=info", Level::INFO)
                .into()
        }))
        .init();
...
async fn health_check(
    ...
    let count = state.request_counter.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
...
async fn create_tdf(
    ...
    state.request_counter.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
    ...
    // Set policy
    manifest.set_policy(&serde_json::to_string(&request.policy)
    ...
    let tdf_data = std::fs::read(&temp_path)
        .map_err(|e| anyhow::anyhow!("Failed to read TDF file: {}", e))?;
...
async fn read_tdf(
    ...
    let tdf_data = base64::engine::general_purpose::STANDARD.decode(&request.tdf_data)
...
async fn decrypt_data(
    ...
    let policy_key = base64::engine::general_purpose::STANDARD.decode(&request.policy_key)
...
async fn validate_policy(
    ...
    let policy_str = entry.manifest.get_policy()
